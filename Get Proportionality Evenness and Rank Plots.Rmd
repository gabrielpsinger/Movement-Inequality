...
assign_bins <- function(df_motility) { #Easy to switch from motility to proportionality
  ...
  for (y in all_years) {
    ...
    #rounded_max <- round_up(max_rkm, 50) # this is chris's original code, the function doesn't work and I can't find where it came from
    rounded_max <- dpylr::round_any(max_rkm, 50, f = ceiling) # Gabe's attempt at replacing the above line.
...
# viridis(5, begin = 0.2)
# "#414487FF" "#2A788EFF" "#22A884FF" "#7AD151FF" "#FDE725FF"

# cividis(5)
#  "#00204DFF" "#414D6BFF" "#7C7B78FF" "#BCAF6FFF" "#FFEA46FF"

#scales::seq_gradient_pal('dodgerblue3', 'darkorange3')(seq(0,1,length.out=5))
# "#1874CD" "#7B70A0" "#A16D73" "#BA6946" "#CD6600"
...
create_lorenz_rank <- function(df_proportion) {
 ...
 # cc <- c("#000080", "#2790DE", "#00BFFF", "#2790DE","#2A62BE", "#000080","#2A62BE", "#2790DE", "#00BFFF", "#00BFFF", "#2A62BE", "#000080" ) # blues
   ...
   cc <-  c("dodgerblue3",  "#BCAF6FFF", "chocolate3", "#BCAF6FFF","lightsteelblue1",  "dodgerblue3","lightsteelblue1", "#BCAF6FFF", "chocolate3", "chocolate3", "lightsteelblue1", "dodgerblue3" ) # blue to orange
  
  #cc <-   c("#440154FF", "#5DC863FF", "#FDE725FF", "#5DC863FF","#21908CFF", "#440154FF","#21908CFF", "#5DC863FF", "#FDE725FF", "#FDE725FF", "#21908CFF", "#440154FF" )# viridis
  
  #cc <- c("#414487FF", "#7AD151FF", "#FDE725FF", "#7AD151FF","#22A884FF", "#414487FF","#22A884FF", "#7AD151FF", "#FDE725FF", "#FDE725FF", "#22A884FF", "#414487FF" ) # viridis 0.2
  
    #cc <- c("#00204DFF", "#BCAF6FFF", "#FFEA46FF", "#BCAF6FFF","#7C7B78FF", "#00204DFF","#7C7B78FF", "#BCAF6FFF", "#FFEA46FF", "#FFEA46FF", "#7C7B78FF", "#00204DFF" ) # cividis
  
  
  labs <- tibble(yr = 2006:2017, x = c(1, 13, 38, 46, 59, 78, 87, 94, 99, 103.5, 109, 115), 
                      y = c(0.3577, 0.1805, 0.3264, 0.2285, 0.1694, 0.2198, 0.3269, 
                            0.4913, 0.6699, 0.2832, 0.2231, 0.7014)) # added for labels
  ...
  a <-ggplot() + 
   ...
   ylab('') +
    ...
    scale_x_continuous(labels = (2006:2017), breaks = c(5, 20, 42, 50, 68, 82, 90, 96, 100, 104, 
                                                        111, 116)) + # new line
    theme_bw() +
  ...
  b <- ggplot(df_lorenz, aes(x=p, y=L, group =  as.character(year))) + geom_line() +
    #scale_color_manual(values = cc) +  #also swapped color for group from above
  ...
  combined_plot <- grid.arrange(a, b, left = textGrob("Total Movement", rot = 90, gp = gpar(fontsize = 16)))
...
new_det <- read_csv("data/Final_Green_Sturgeon.csv", 
...
gs_test <- get_species_motility(new_det, 'Green_Sturgeon', 1800, df_all_years)

gs_test_bins <- assign_bins(gs_test)
```
